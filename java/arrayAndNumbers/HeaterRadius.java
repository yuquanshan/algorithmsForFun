...
public class HeaterRadius{
	public static int findRadius(int[] houses, int[] heaters){
		Arrays.sort(houses);
		Arrays.sort(heaters);
		int pt = 0; // the position of first heater whose position is larger or equal to current house
		int radius = 0;
		for(int i = 0; i<houses.length; i++){
			while(pt < heaters.length && heaters[pt]<houses[i])
				pt++;
			int p1 = pt-1;
			int r = Integer.MAX_VALUE;
			if(p1 >= 0)
				r = houses[i]-heaters[p1];
			if(pt < heaters.length)
				r = Math.min(r,heaters[pt]-houses[i]);
